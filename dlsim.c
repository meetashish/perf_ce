...
int32_t pack_iq(int16_t i, int16_t q) {
  int32_t iq = (((int32_t) q) << 16) | (((int32_t) i) & 0xFFFF);
  return iq;
}
...
void DL_channel(RU_t *ru,PHY_VARS_UE *UE,uint subframe,int awgn_flag,double SNR, int tx_lev,int hold_channel,int abstx, int num_rounds, int trials, int round, channel_desc_t *eNB2UE[4],
  ...
  if (awgn_flag == 0) {
    ...
    if (perfectce == 1) {
      /// IISC ADDED      
      double *ch_t;     // channel coefficients (floating point) in time domain
      int16_t *ch_16t;  // channel coefficients (Q15 format) in time domain
      int16_t *ch_16f;  // channel coefficients (Q15 format) in frequency domain
      ...
      switch (UE->frame_parms.ofdm_symbol_size) {
        ...
        case 512:
          ch_t = (double*)calloc(7168*2, sizeof(double));     // I = 512*14, Q = 512*14
          ch_16t = (int16_t*)calloc(7168*2, sizeof(int16_t));
          ch_16f = (int16_t*)calloc(7168*2, sizeof(int16_t));

          // Copy channel esimates in ch_t
          for (size_t i = 0; i < 7168*2; i++) {
            ch_t[i] = ((i & 1) == 0)? 
                      eNB2UE[round]->ch[0]->x : // real part of channel coefficient in floating point
                      eNB2UE[round]->ch[0]->y;  // imag part of channel coefficient in floating point

            ch_16t[i] = ((i & 1) == 0)? 
                        (eNB2UE[round]->ch[0]->x)*(1 << 15) : // real part of channel coefficient in Q15
                        (eNB2UE[round]->ch[0]->y)*(1 << 15);  // imag part of channel coefficient in Q15 
          }
          /// Compute FFT
          dft512(ch_16t, ch_16f, 1);

          for (size_t i = 0; i < 7168; i = i + 2) {
            printf("Channel Coefficients[%ld]: (%lf, %lf) ==> Q15: (%d, %d) <==> (%d, %d)\n", 
                    i, ch_t[i], ch_t[i+1], ch_16t[i], ch_16t[i+1], ch_16f[i], ch_16f[i+1]);
          }
          /// Save them in a file
          for (size_t i = 0; i < 7168*2; i++) {
            FILE * fcht = fopen("Channelt.m", "a");
            fprintf(fcht, "%d; ...\n", ch_16t[i]); fclose(fcht);
            FILE * fchf = fopen("Channelf.m", "a");
            fprintf(fchf, "%d; ...\n", ch_16f[i]); fclose(fchf);
          }
          
          for (size_t i = 0; i < 7168; i++) {
            UE->common_vars.perf_chestF[0][i] = pack_iq(ch_16f[0], ch_16f[1]);
            printf("Perfect CE_Freq[%d] = %d\n", i, UE->common_vars.perf_chestF[0][i]);
            // // (int16_t*)&UE->common_vars.perf_chestF[0][i] = ((i & 1) == 0)? ch_16f[0] : ch_16f[1];
            // int16_t *a = (int16_t*) &UE->common_vars.perf_chestF[0][i];
            // a = ((i & 1) == 0)? ch_16f[0] : ch_16f[1];
          }break;
  ...
  for (i=0; i<2*UE->frame_parms.samples_per_tti; i++) {
    for (aa=0; aa<UE->frame_parms.nb_antennas_rx; aa++) {
      // FILE *fnoise = fopen("Noise.m","a");
      // FILE *fRecv = fopen("Receive.m","a");
      
      double nre = sqrt(sigma2/2)*gaussdouble(0.0,1.0);
      double nim = sqrt(sigma2/2)*gaussdouble(0.0,1.0);
      // printf("IISC Noise = [%f, %f] for i = %d and nrx = %d\n", nre, nim, i, aa);
      // fprintf(fnoise, "%f, %f; ...\n", nre, nim); fclose(fnoise);
      ...
      ((short *) UE->common_vars.rxdata[aa])[(2*subframe*UE->frame_parms.samples_per_tti)+2*i+1] =
        (short) (r_im[aa][i] + (iqim*r_re[aa][i]) + nim);
        // fprintf(fRecv,"%d, %d; ...\n", (short) (r_re[aa][i] + nre), (short) (r_im[aa][i] + (iqim*r_re[aa][i]) + nim)); fclose(fRecv);
    }
  }
}
...
int main(int argc, char **argv) {
  ...
  uint8_t transmission_mode=1,n_tx_port=1,n_tx_phy=1,n_rx=1;
  int eNB_id = 0;
  unsigned char round;
  unsigned char i_mod = 2;
  int NB_RB;
  SCM_t channel_model=Rayleigh1; //EVA; // Rayleigh1;
  ...
  char channel_model_input[10]="I"; //F I
  int TB0_active = 1;
  ...
  static paramdef_t options[] = {
    ...
    { "mcs1", "The MCS for TB 1", 0, iptr:&mcs1,  defintval:28, TYPE_INT, 0 },
    ...
    { "snr", "Starting SNR, runs from SNR to SNR+%.1fdB in steps of %.1fdB. If n_frames is 1 then just SNR is simulated and MATLAB/OCTAVE output is generated", dblptr:&snr0,  defdblval:20.0, TYPE_DOUBLE, 0 },
    ...
    { "RNumRound", "Number of HARQ rounds (fixed)",0, iptr:&num_rounds,  defintval:1, TYPE_INT, 0 },
    ...
    { "yn_tx_phy","Number of TX antennas used in eNB",0, iptr:NULL,  defintval:1, TYPE_INT, 0 },
    ...
    { "zn_rx", "Number of RX antennas used in UE",0, iptr:NULL,  defintval:1, TYPE_INT, 0 },
  ...
  */
  ...
  for (ch_realization=0; ch_realization<n_ch_rlz; ch_realization++) {
    printf(" IISC: Channel Realization Index = %d \n", ch_realization);
    if(abstx) {
      printf("IISC: Abstraction Channel Realization Index = %d \n", ch_realization);
    }

    for (SNR=snr0; SNR<snr1; SNR+=snr_step) {
      printf("IISC: SNR0 = %f, SNR Index = %f, SNR1 = %f, Channel Realization Index = %d\n", snr0, SNR, snr1, ch_realization);
      ...
      for (trials = 0; trials<n_frames; trials++) {
        printf(" IISC: Number of frames = %d, Number of trials = %d\n", n_frames, trials);
        //printf("Trial %d\n",trials);
        fflush(stdout);
        round=0;
        //if (trials%100==0)
        ...
        while ((round < num_rounds) && (UE->dlsch[UE->current_thread_id[subframe]][eNB_id][0]->harq_ack[subframe].ack == 0)) {
          ...
          printf("IISC Channel Coefficients = [%f, %f]\n", eNB2UE[0]->ch[0][0].x, eNB2UE[0]->ch[0][0].y);
          ...
          slot_fep(UE,
                   ...
                   0,
                   0);
          for (int p = 0; p < frame_parms->ofdm_symbol_size; p++) {
            for (aa=0; aa<frame_parms->nb_antennas_rx; aa++) {
              FILE *fchest = fopen("Chest.m","a");
              short Re = ((int16_t*)UE->common_vars.common_vars_rx_data_per_thread[UE->current_thread_id[subframe]].dl_ch_estimates_time[eNB_id][aa])[(p<<1)];
              short Im = ((int16_t*)UE->common_vars.common_vars_rx_data_per_thread[UE->current_thread_id[subframe]].dl_ch_estimates_time[eNB_id][aa])[1+(p<<1)];
              fprintf(fchest, "%d, %d, %f, %f; ...\n", Re, Im, (float) Re/32768, (float) Im/32768); fclose(fchest);
              // printf("Estimated Channel Coefficients = [%d, %d] = [%f, %f]\n", Re, Im, (float) Re/32768, (float) Im/32768);
            }
          }
          ...
          char filename[100];
          sprintf(filename, "ERROR_%d_%d_%d.m", N_RB_DL, mcs1, (int)SNR);
          ...
          if (UE->dlsch[UE->current_thread_id[subframe]][eNB_id][0]->harq_ack[subframe].ack == 1) {
            ...
            if (n_frames==1){
              ...
              FILE *ferror = fopen(filename,"a");
              fprintf(ferror, "%f, ...\n", (double)bit_errors/coded_bits_per_codeword); fclose(ferror);
            }
