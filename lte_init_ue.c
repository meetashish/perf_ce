...
int init_lte_ue_signal(PHY_VARS_UE *ue,
  ...
  // Channel estimates
  common_vars->perf_chestF   = (int32_t **)malloc16( fp->nb_antennas_rx*sizeof(int32_t *) );  // IISC Ashish
  for (i=0; i<fp->nb_antennas_tx; i++) {                                                      // IISC Ashish
    common_vars->perf_chestF[i] = (int32_t *)malloc16_clear( fp->ofdm_symbol_size*fp->symbols_per_tti*sizeof(int32_t) ); // IISC Ashish
  }                                                                                                                         // IISC Ashish
  for (eNB_id=0; eNB_id<7; eNB_id++) {
    for (th_id=0; th_id<RX_NB_TH_MAX; th_id++) {
      ...
      common_vars->common_vars_rx_data_per_thread[th_id].perf_dl_ch_estmatesF[eNB_id] = (int32_t **)malloc16_clear(8*sizeof(int32_t *));  // IISC Ashish
    ...
    for (i=0; i<fp->nb_antennas_rx; i++)
      for (j=0; j<4; j++) {
        ...
        for (th_id=0; th_id<RX_NB_TH_MAX; th_id++) {
          ...
          common_vars->common_vars_rx_data_per_thread[th_id].perf_dl_ch_estmatesF[eNB_id][idx] = (int32_t *)malloc16_clear( sizeof(int32_t)*fp->symbols_per_tti*(fp->ofdm_symbol_size+LTE_CE_FILTER_LENGTH) );
