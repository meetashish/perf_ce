...
// extern int dl_ch_est;
// printf("%d", dl_ch_est);

void copy_pilot_2_chest(int16_t *chest0, int16_t *chest1, int16_t *chest_interp, uint32_t N) {
    unsigned int i = 0;
    for(i = 0; i < N; i = i + 2) {
        chest_interp[i] = *chest0;
        chest_interp[i+1] = *chest1;
        // printf("Estimated Channel Coefficients = [%d, %d], Interpolated Channel Coefficients = [%d, %d]\n", 
        //         *chest0, *chest1, chest_interp[i], chest_interp[i+1]);
    }
}
...
int lte_dl_channel_estimation(PHY_VARS_UE *ue,
  ...
  for (aarx=0; aarx<ue->frame_parms.nb_antennas_rx; aarx++) {
    ...
    // for (size_t i = 0; i < 200; i = i + 2)  {
    //   printf("IISC Pilots = [%d, %d]\n", pil[i], pil[i+1]);
    // }
    ...
    if ((ue->frame_parms.N_RB_DL==6)  ||
      ...
      //      ch[1] = -128;
#endif
      // printf("IISC pilot 0 : rxF - > (%d,%d) ch -> (%d,%d), pil -> (%d,%d), RB = %d \n",
      //         rxF[0],rxF[1],ch[0],ch[1],pil[0],pil[1], NRB);
      copy_pilot_2_chest(&ch[0], &ch[1], dl_ch, 24);
      pil+=4;    // Re Im
      rxF+=24;
      dl_ch+=24;
      // multadd_real_vector_complex_scalar(fl,
      ...
      //      ch[1] = -128;
#endif
      // printf("IISC pilot 0 : rxF - > (%d,%d) ch -> (%d,%d), pil -> (%d,%d) \n",rxF[0],rxF[1],ch[0],ch[1],pil[0],pil[1]);
      // copy_pilot_2_chest(&ch[0], &ch[1], dl_ch, 16);
      // // multadd_real_vector_complex_scalar(f2l2,
      // //                                    ch,
      // //                                    dl_ch,
      // //                                    24);
  ...
  // Copy perfect channel estimates into dl_ch_esimates
  memcpy(ue->common_vars.common_vars_rx_data_per_thread[ue->current_thread_id[Ns>>1]].dl_ch_estimates[eNB_offset],
         ue->common_vars.perf_chestF[0], 221);

  for (int q = 0; q < 7168; q++) {
    printf("Channel Freq CE[%d] = (%d, %d)\n", q,
    ue->common_vars.perf_chestF[0][q], 
    ue->common_vars.common_vars_rx_data_per_thread[ue->current_thread_id[Ns>>1]].dl_ch_estimates[eNB_offset][q]);
  }
